...
export default function SearchDropdown({ results, loading, query }: Props) {
  ...
  return (
    <div className="absolute left-0 top-full z-50 mt-1 w-full rounded-md border bg-popover shadow-md">
      {loading ? (
        <p className="px-3 py-4 text-center text-sm text-muted-foreground">검색 중...</p>
      ) : !hasResults ? (
        ...
        <div className="p-1">
          {results.tasks.length > 0 && (
            <div>
              <p className="px-3 py-1 text-xs font-semibold text-muted-foreground">Tasks</p>
          ...
          {results.teams.length > 0 && (
            <div>
              <p className="px-3 py-1 text-xs font-semibold text-muted-foreground">Teams</p>
